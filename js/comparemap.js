...
function geneCompare(geneValues)
{
	...
	geneData = randomData(width1, height,genes,speciesName,geneName);
	...
	//var x_limit = Math.min(maxLength, width)
   ...
   var textlabel=  labelRect
    ...
    var svgCompare1 = d3.select('#compareViz').append("svg")
                    .style("width", width1 + margin.left + margin.right)
                    .style("height", height + margin.top + margin.bottom)
                    .attr("class", "vis")
					.style("background","#777777")
                    .call(zoom)
...
function zoomed()
{
    ...
    var col1 = row1.selectAll(".cell")
                ...
                .attr("transform",function(d,index){
                	...
                	return "translate("+x(index)+4+","+d.y+")";
                })
                //.attr("width", width)
                .append("svg:rect")
                 .attr("class", "cell")
